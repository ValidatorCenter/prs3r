...
func startWorkerBNode(workerNum uint, in <-chan ms.BlockResponse) {
	for retBlck := range in {
		for _, pV1 := range retBlck.Validators {
			if pV1.Signed == false {
				// пропустили блок!
				...
				a.Blocks = append(a.Blocks, s.BlocksStory{ID: uint32(retBlck.Height), Type: "AbsentBlock"})
				if !addNodeBlockstorySql(dbSQL, &a) {
					log("ERR", "[w_node.go] startWorkerBNode(addNodeBlockstorySql) AbsentBlock", "")
				}
				log("STR", fmt.Sprintf("AbsentBlock:: %d - %s", retBlck.Height, pV1.PubKey), "")
			}
		...
		addBlockValidSqlArr(dbSQL, &retBlck)
